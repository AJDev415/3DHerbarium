...
const Viewer: React.FC<ViewerProps> = ({
    ...
    // TRACK TOTAL PROGRESS MANUALLY
    let totalBytes = 0;
    let loadedBytes = 0;

    function trackProgress(event: any) {
        if (!event.lengthComputable) return;  // skip if server doesn't send size
        loadedBytes += event.loaded;
        const percent = Math.min(100, Math.round((loadedBytes / totalBytes) * 100));
        setLoadingProgress(percent);
    }

    // STEP 1: Get MTL size FIRST (HEAD request)
    ...
    useEffect(() => {
        ...
        const loadModel = async () => {
            ...
            const mtlSize = await getFileSize(mtlUrl);
            const objSize = await getFileSize(objUrl);
            totalBytes = mtlSize + objSize;
            ...
            fileLoader.load(mtlUrl, (mtlText) => {
                if (canceled) return
                const mtlLoader = new MTLLoader();
                const materials = mtlLoader.parse(mtlText as string, mtlDir)
                materials.preload();

                const loader = new OBJLoader()
                loader.setMaterials(materials)

                fileLoader.load(objUrl, (objText) => {
                    if (canceled) return
                    const object = loader.parse(objText as string);

                    // Add to scene and finish loading
                    if (!canceled) {
